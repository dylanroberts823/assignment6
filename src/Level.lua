...
function Level:init()
    ...
    --a constant to reset when we can have multiple aliens
    ...
    function endContact(a, b, coll)
      --once the contact has happened, prevent any splitting from happening
...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        ...
        local aliensStopped = true

        for k, alien in pairs(self.aliens) do
          local xPos2, yPos2 = alien.body:getPosition()
          local xVel2, yVel2 = alien.body:getLinearVelocity()
          if xPos2 < 0 or (math.abs(xVel2) + math.abs(yVel2) < 1.5) then
            aliensStopped = true
          else
            aliensStopped = false
          end
        ...
        if (xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5)) and aliensStopped then
            self.launchMarker.alien.body:destroy()
            self.launchMarker = AlienLaunchMarker(self.world)

            -- re-initialize level if we have no more aliens
            if #self.aliens == 0 or #self.aliens == 2 then
...
function Level:multiplyAliens()
  if self.launchMarker.launched and not self.hasContact then
    -- track current alien
    local xPos, yPos = self.launchMarker.alien.body:getPosition()
    local xVel, yVel = self.launchMarker.alien.body:getLinearVelocity()

    --spawn two aliens
    table.insert(self.aliens, Alien(self.world, 'round', xPos, yPos, 'Player'))
    table.insert(self.aliens, Alien(self.world, 'round', xPos, yPos, 'Player'))

    --change their settings
    for k, alien in pairs(self.aliens) do
      --modify the angles according to the alien
      if k == 2 then
        alien.body:setLinearVelocity(xVel, yVel + 5)
      elseif k == 3 then
        alien.body:setLinearVelocity(xVel, yVel - 5)
      end

      -- make the aliens are pretty bouncy
...
function Level:render()
    ...
    --change victory to account for us adding our player aliens to the scene
    if #self.aliens == 0 or #self.aliens == 2 then
